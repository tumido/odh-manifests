...
echo -n ".. Removing the teporary clone"
rm -rf $tmp_dir
echo -e "\r ✓ "
...
echo ".. Copying release images to Quay.io"
for image in argocli argoexec workflow-controller; do
    echo -n "  .. [$image]"
    podman pull -q docker.io/argoproj/$image:$version > /dev/null
    podman tag argoproj/$image:$version quay.io/opendatahub/$image:$version
    podman push -q quay.io/opendatahub/$image:$version > /dev/null
    echo -e "\r   ✓"
done
